// unix_server_looped.c: demonstration of Unix domain sockets, local sockets
// which are based on a file on the file system as the 'address' of
// the socket. The server listens for connections from clients and
// echos back the text that they send. Only a single client at a time
// is serviced but the server operates in a LOOP so will later accept
// other clients.
// 
